...
export class ArtifactCache {
  ...
  async fetchWithCache(url: string, responseContent?: BodyInit) {
...
export class Instance implements Disposable {
  ...
  *
  ...
  private async fetchNDArrayCacheInternal(
    ...
    const reportCallback = (iter: number, file?: File) => {
      // report
    ...
    for (let i = 0; i < list.length; ++i) {
      const fileBody = files?.find((file) => list[i].dataPath === file.name);
      reportCallback(i, fileBody);
  ...
  getFileNameFromResponse(response: Response, defaultUrl?: string): string | null {
    ...
    // If Content-Disposition header is not present or doesn't contain a filename, try to extract from the URL
    const url = new URL(response.url || defaultUrl);
